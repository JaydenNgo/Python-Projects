...
a = 96
notation = {chr(a+i)+str(j):[-375+(80*i),-340+(80*j)] for i in range(1,9) for j in range(1,9)}
...
piecePositions = {i:"empty" for i in notation}
...
for p in pieces: ## CREATE PIECES FROM PIECES DICTIONARY
    ...
    elif pieces[p][0] in all_pieces:
        exec(str(p) + '.shape' + '("' + str(pieces[p][0]) + '")')
...
def checkSquare(x,y, square): ## MOVE TO SQUARE LOGIC
    ...
    if selectedPiece != None and ((piecePositions[selectedSquare] == 'empty') or pieces[piecePositions[selectedSquare]][2] != activeTeam): ##CHECK IF PIECE IS SELECTED AND SELECTED SQUARE IS EMPTY
        ...
        if pieces[selectedPiece][0] == 'knight':
            ...
            nss = notation[selectedSquare]
            npss1 = notation[(pieces[selectedPiece][1])]
            corners = [-2,-1,1,2]
            for i in corners:
                for j in corners:
                    if abs(i) == abs(j): continue
                    else:
                        if ((nss[0] == npss1[0] + i*80) and (nss[1] == npss1[1] + j*80)): 
                            return True
            else: 
                return False
