...
# make it so all of the counts are class attributes instead of globals
...
class Board:
  ...
  def multiscan(self, solution):
    i,j = self.find_empty()
    global multi_count; multi_count += len(self.board[i][j])
...
if test:
  ...
  for i in range(times):
    ...
    print(f"~ {multi_count} boards checked total")
    print(f"Find empty checks {find_empty_count:,}")
    print(f"Clear checks {clear_count:,}")
    print("\n\n\n")
    original.print_board_thirds()
  avg_time = total_time/times
  print(f"Average Runtime: {avg_time//60} min {avg_time%60:.3} s\n")
