...
class Player:
  ...
  # Returns list of colors you do have full set of
  ...
  def buy_house(self):
    ...
    for obj in self.property.values():
      if isinstance(obj, Property) and obj.color in full_sets:
        print(f"{c} {obj.name+':':25} {obj.build_cost}, {obj.house_count}")
        c += 1
    print("Which property would you like to buy a house for?\n")
    #choice = input("Which property would you like to buy a house for?\n")
    #Complete
  ...
  def buy_property(self, deed, buyables):
    ...
    if buyables == railroads:
      # Need to Change all railroads owned not just the new one
  ...
  # Rolls die, moves player, and calls Landed function
  def roll(self):
    ...
    if (self.pos >= 40):
      self.pos %= 40
      self.money += 200
      print(f"{self.name} has passed GO and Earned $200, now has ${self.money}")

    # Run landed code
    self.landed(rolled=total, doubles_occured=doubles)


  # Whenver a player lands on a tile
  def landed(self, rolled = None, doubles_occured = False):
    ...
    for buyables in (properties, railroads, utilities):
      if landed_tile in buyables:
        ...
        if deed.isowned(): #owned pay rent
          ...
          if self == p2:
            print(f"{self.name} owns {landed_tile}")
            break
          print(f"{landed_tile} is owned by {deed.owner}")
          if deed.ismortgaged:
          ...
          if buyables == utilities:
            if rolled == None:
              dice1, dice2 = dice_roll()
              rolled = dice1 + dice2
            deed.rent = (6*p2.num_utility-2)*(rolled)
          ...
          print(f"{landed_tile} is not owned and costs ${deed.price}")
          # Add check if you have enough money
          # prompt = input(f"Would you like to buy {landed_tile}? (Y/N) ").upper()
          prompt = "Y"
          if prompt in ("Y","YES"):
            ...
            self.property[landed_tile] = deed
            self.buy_property(deed, buyables)
            
          else: # Auction
            #break
            print(f"\n{landed_tile} is up for Auction!")
            auction_price = 10
            auctioner = None
            while True:
              passes = 0
              ...
              for player in in_auction:
                if auctioner != None:
                  if player.name == auctioner.name:
                    passes += 1
                    continue
                print(f"Current Price is {auction_price} by {auctioner.name if auctioner else 'the Bank'}\n")
    ...
    if (doubles_occured):
      print(f"{self.name} rolled doubles, they go again\n")
      self.roll()
...
all_colors = ['Brown', 'Light Blue', 'Pink', 'Orange', 'Red', 'Yellow', 'Green', 'Dark Blue']
